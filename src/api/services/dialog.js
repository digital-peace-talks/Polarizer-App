...
module.exports.createDialog = async (options) => {
  ...
  try {
    const opinion = await Opinion.findOne({
      _id: mongoose.Types.ObjectId(options.opinion),
    });
    options.recipient = opinion.user;
    const initiatorOpinion = await Opinion.findOne({
      topic: mongoose.Types.ObjectId(options.topic),
      user: mongoose.Types.ObjectId(options.initiator),
    });
...
async function findExtraDialog(firstDialog) {
  ...
  var opinions = await Opinion.find({
    topic: mongoose.mongo.ObjectId(opinion.topic),
  });
  ...
  for (var i in opinions) {
    ...
    for (j in dialogs) {
      if (
        dialogs[j].initiator.toString() == firstDialog.recipient.toString() &&
...
module.exports.getDialog = async (options, getSet) => {
  ...
  try {
    ...
    if (getSet) {
      ...
      if (result2) {
        if (
          result.crisises[1].startDate.valueOf() <
          result2.crisises[1].startDate.valueOf()
        ) {
        ...
        if ("user" in options) {
          if (result2.initiator.id.toString("hex") == options.user.user.id) {
            await Dialog.findByIdAndUpdate(dialogId, {
              initiatorTimestamp: now,
            });
          } else if (
            result2.recipient.id.toString("hex") == options.user.user.id
          ) {
            await Dialog.findByIdAndUpdate(dialogId, {
...
module.exports.getDialogListAll = async (options) => {
  ...
  try {
    ...
    worker = await Dialog.find({
      $or: [{ recipient: option.userId }, { initiator: option.userId }],
    });
...
module.exports.getDialogListNew = async (options) => {
  ...
  try {
    var worker = {};
    worker = await Dialog.find({
      $or: [{ initiator: options.userId }, { recipient: options.userId }],
    });
    for (var i in worker) {
      var collection = {};
      collection.dialog = worker[i].id;
      ...
      var worker2 = await Topic.findOne({
        _id: mongoose.Types.ObjectId(worker[i].topic),
      });
      collection.topic = worker2.content;
      worker2 = await Opinion.find({
        _id: mongoose.Types.ObjectId(worker[i].opinion),
      });
      collection.recipientOpinion = worker2.content;
      worker2 = await Opinion.find({
        _id: mongoose.Types.ObjectId(worker[i].initiatorOpinion),
      });
...
module.exports.getDialogList = async (options) => {
  ...
  try {
    var worker = {};
    worker = await Dialog.find({ initiator: options.userId }).populate(
      "opinion"
    );
    for (var i = 0; i < worker.length; i++) {
      var collection = {};
      ...
      collection.unreadMessages = 0;
      var worker2 = await Topic.findOne({
        _id: mongoose.Types.ObjectId(worker[i].opinion.topic),
      }).populate("opinions");
      collection.topic = worker2.content;
    ...
    worker = await Dialog.find({ recipient: options.userId }).populate(
      "opinion"
    );
    for (var i = 0; i < worker.length; i++) {
      var collection = {};
      ...
      collection.unreadMessages = 0;
      if (
        (collection.status == "PENDING" || collection.status == "CRISIS") &&
...
module.exports.postMessage = async (options) => {
  ...
  try {
    ...
    if (
      ...
      config.api.maxMessages * dialog.extension
    ) {
      throw {
        status: 500,
