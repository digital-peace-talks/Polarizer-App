...
module.exports.opinionOpinionsExist = async (options, oid) => {
  if (options.body.topicId && oid) {
    var opinions = await Opinion.findOne({
      topic: mongoose.Types.ObjectId(options.body.topicId),
      user: oid._id,
    });
    if (opinions == null) {
      return false;
  ...
  if (options.body.id) {
    try {
      ...
      for (var i in topic.opinions) {
        var opinion = await Opinion.findOne({
          _id: topic.opinions[i],
        }).populate("topic", { content: 1 });
        if (opinion) {
      ...
      for (var i in opinions) {
        ...
        for (var j in dialogs) {
          ...
          if (dialog) {
            ...
            var opinionInitiator = Lo_.find(opinions, {
              user: dialog.initiator,
            });

            var opinionRecipient = Lo_.find(opinions, {
              user: dialog.recipient,
            });

            var crisisInitiator = Lo_.find(dialog.crisises, {
              initiator: dialog.initiator,
            });
            var crisisRecipient = Lo_.find(dialog.crisises, {
              initiator: dialog.recipient,
            });
...
module.exports.opinionPut = async (options) => {
  ...
  try {
    result = await Opinion.findByIdAndUpdate(options.opinionId, options.body);
