...
module.exports.userReclaim = async (options) => {
  var status = 200;
  const userByPhraseGuess = await User.findOne({
    phrase: options.body.phraseGuess,
  });
  const newPhrase = await User.findOne({ phrase: options.body.newPhrase });
  const userBydptUUID = await User.findOne({
  ...
  // case A + D
  if (userByPhraseGuess != null && userBydptUUID === null) {
    console.log("--------------------------");
    ...
    // case B + D
    if (newPhrase === null && userBydptUUID === null) {
