...
const connection = mongoose.connect("mongodb://localhost:27017/dpt-dev", {
...
async function updateDialog(dialog) {
  try {
    ...
    var opinion2 = await Opinion.findOne({
      user: dialog.initiator,
      topic: opinion.topic,
    });
    await Dialog.updateOne(
      { _id: dialog._id },
      { initiatorOpinion: opinion2._id }
    );
  } catch (error) {
    throw {
      status: 500,
