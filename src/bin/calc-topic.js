  ...
  for (var i in topic.opinions) {
    ...
    for (var j in dialogs) {
      for (var k in dialogs[j].crisises) {
        console.log(
          "opinion: " +
            users[dialogs[j].crisises[k].initiator] +
            " rating: " +
            dialogs[j].crisises[k].rating
        );
        opinions[users[dialogs[j].crisises[k].initiator]].rating +=
          dialogs[j].crisises[k].rating;
  ...
  for (var i in opinions) {
    console.log(
      "opinion: " +
        opinions[i].content +
        " rating: " +
        mapRange([-1, 1], [0, 1], opinions[i].rating / opinions[i].num)
    );
  }
}
