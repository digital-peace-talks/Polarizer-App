...
async function deleteTopic(topicId) {
  ...
  for (var i in topic.opinions) {
    ...
    for (var j in dialogs) {
      console.log(
        "DIALOGS - delete: " + util.inspect(dialogs[j], { depth: 5 })
      );
...
async function deleteOpinion(opinionId) {
  try {
    ...
    var dialogs = await Dialog.find(
      { $or: [{ opinion: opinionId }, { initiatorOpinion: opinionId }] },
      { _id: 1 }
    );
    for (i in dialogs) {
...
app.listen(HTTP_PORT, async () => {
  connection = await mongoose.connect("mongodb://localhost:27017/dpt-dev", {
