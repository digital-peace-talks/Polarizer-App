  ...
  const guidePosition = [
    {
      ...
      right: "auto",
      topMark:
        document.getElementById("guideNextBtn").getBoundingClientRect().top +
        "px",
      leftMark:
        document.getElementById("guideNextBtn").getBoundingClientRect().left +
        "px",
      buttonLeftName: "",
    ...
    },
    {
      ...
      right: "auto",
      topMark:
        document.getElementById("new-topic-btn").getBoundingClientRect().top +
        "px",
      leftMark:
        document.getElementById("new-topic-btn").getBoundingClientRect().left +
        "px",
      buttonLeftName: "BACK",
      buttonRightName: "NEXT",
    ...
    },
    {
      ...
      right: "auto",
      topMark:
        document.getElementById("dialogues-btn").getBoundingClientRect().top +
        "px",
      leftMark:
        document.getElementById("dialogues-btn").getBoundingClientRect().left +
        "px",
      buttonLeftName: "BACK",
      buttonRightName: "NEXT",
...
function onWebSocketUpdate(restObj) {
  ...
  //Refactor: Does this even do anything?
    ...
    if (
      restObj.path.startsWith("/opinion/") &&
...
function validateRestObj(restObj) {
  if (!restObj) {
    return false;
  }
...
function isCurrentDialog(restObj) {
  return (
    currentDialog && restObj.path == "/dialog/" + currentDialog.dialog + "/"
  );
}

function openDialogFormCurrentDialog(restObj) {
...
function setCurrentDialog(restObj) {
  return (
    currentDialog && restObj.path == "/dialogSet/" + currentDialog.dialog + "/"
  );
}
function openDialogFormDialogSet(restObj) {
...
function replaceEditOpinionWithNewOpinion() {
  jQuery("#edit-btn").css("display", "none");
  jQuery("#new-opinion-btn").show();
}
function opinionAlreadyExists(restObj) {
  if (restObj.data.value == false) {
    opinionForm();
  } else {
    replaceNewOpinionWithEdit();
    topicUserOpinion = restObj.data.value;
  }
}
function webSocketHandleGet(restObj) {
  if (isCurrentDialog()) {
    openDialogFormCurrentDialog(restObj);
  }
  if (setCurrentDialog()) {
    openDialogFormDialogSet(restObj);
  }
  console.log(restObj.path);
  switch (restObj.path) {
    case "/topic/":
      if (currentScene.name == "topicScene") {
        loadTopics(restObj);
    ...
    case "/metadata/search/":
      if (restObj.path.startsWith("/metadata/search/")) {
    ...
    case "/opinion/" + currentTopic + "/":
      if (currentScene.name == "opinionScene") {
        ...
        dpt.opinionOpinionsExist(currentTopic);
      }
      break;
    case "/opinion/opinionsExist/":
      opinionAlreadyExists(restObj);
      break;
    case "/dialog/list/":
...
function main() {
  ...
  socket.on("3d", function (update) {
    if (update.event == "connect" && update.avatar != socket.id) {
