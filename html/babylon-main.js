  ...
  if (camera) {
    ...
    setInterval(() => {
      socket.emit("3d", {
        event: "update",
        avatar: name,
        avatarPos: currentScene.cameras[0].position,
      });
    }, 1000);
  } else {
...
function setBabylonScheme() {
  switch (whoami.user.preferences.colorScheme) {
    ...
    case DPTGlobal.COLORS_bright:
      currentScene.clearColor = new BABYLON.Color3(0.7, 0.9, 1.0);
      break;
    case DPTGlobal.COLORS_skybox:
      currentScene.clearColor = new BABYLON.Color3(
        10 / 255,
        80 / 255,
        119 / 255
      );
    ...
    switch (pointerInfo.type) {
      ...
      case BABYLON.PointerEventTypes.POINTERTAP:
        ...
        if (picked && "dpt" in picked) {
          ...
          // click point
          var click = BABYLON.Vector3.TransformCoordinates(
          ...
          if (click.y < 0.36) {
            if (click.x >= 0 && click.x < 0.36) {
              // invite to chat via proposition
              if (
                "opinionId" in picked.dpt &&
            ...
            if (picked.dpt.status == "CLOSED") {
              for (var i in currentScene.meshes) {
                if (
                  ...
                  currentScene.meshes[i].dpt.initiatorOpinion ==
                    picked.dpt.recipientsOpinion &&
                  currentScene.meshes[i].dpt.recipientOpinion ==
                    picked.dpt.initiatorOpinion
                ) {
