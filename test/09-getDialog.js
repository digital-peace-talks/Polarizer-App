...
function getDialogTest(dialogID)
//-------------------------------------------
    ...
    describe('request', function()
    {
        ...
        {this.timeout(10000);
            await socket.emit("api",
            {
				...
				path: '/dialog/dialogID/', // siehe DTP-openapi
				data:{content: dialogID
				//---------------------------------------
				}
			});
		});
            ...
            if(restObj.method == 'post')
            {
                if(restObj.path == '/user/login/')
                {
					...
					if(restObj.data.message == 'logged in')
                    {
						...
						//------------------------------------------
						getDialogTest("dialogID"); // aufruf der function (args siehe DPT-openapi) 
						//-----------------------------------------
