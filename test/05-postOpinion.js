...
function postOpinionTest(newOpinion, topicID, user) {
//-----------------------------------
	...
	describe('request', function() {
		it('should send string', async function() {
			...
			await socket.emit("api", {
				...
				data: {content:  newOpinion, topic:topicID, user:whoami.user,}	// args für die function
				//------------------------------------
	    ...
	    it('it should receive an anwer', function(done) {
			...
			socket.on('api', async function(payload) {
				...
				//----------------------------------------------------------------------
				assert.equal(payload.data.content,newOpinion,topicID,user);
				//----------------------------------------------------------------------
				...
				if(restObj.method == 'post') {
					if(restObj.path == '/user/login/') {
						...
						if(restObj.data.message == 'logged in') {
							...
							//--------------------------
							postOpinionTest("newOpinion",topicID,whoami.user); // aufruf der function (args siehe DPT-openapi) 
							//--------------------------
